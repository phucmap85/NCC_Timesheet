...
export class PositionService {
  ...
  async getAllPagging(
    ...
    return this.positionRepository.getAllPaging(
  ...
  async createPosition(
    ...
    try {
      const existingName = await this.positionRepository.getPositionByName(name);
      if (existingName) {
        throw new Error(`Position with name ${name} already exists`);
      }
      const existingShortName = await this.positionRepository.getPositionByShortName(shortName);
      if (existingShortName) {
        throw new Error(`Position with short name ${shortName} already exists`);
      }
      const existingCode = await this.positionRepository.getPositionByCode(code);
      if (existingCode) {
      ...
      return await this.positionRepository.savePosition(newPosition);
    } catch (error) {
  ...
  async updatePosition(
    ...
    try {
      let position = await this.positionRepository.getPositionById(id);
      ...
      return await this.positionRepository.savePosition(position);
    } catch (error) {
      throw new BadRequestException(error.message);
    }
  }
  ...
  async deletePosition(id: number): Promise<void> {
    try {
      ...
      try {
        return await this.positionRepository.removePosition(position);
      } catch (error) {
